			...
			m_i2c(device_i2c_num, I2C_ADDRESS)
{
	...
	// configuration
	m_i2c.write_num<uint8_t>(CONFIGURATION_REGISTER_A, 0x6 << 2); // 50 tims a second
...
Hmc5843Compass::vector_t Hmc5843Compass::get_data() {
	...
	// read the data
	vector_t fans;
	fans[0] = (float)m_i2c.read_num<uint16_t>(X_READ_ADDRES);
	fans[1] = (float)m_i2c.read_num<uint16_t>(Y_READ_ADDRES);
	fans[2] = (float)m_i2c.read_num<uint16_t>(Z_READ_ADDRES);
