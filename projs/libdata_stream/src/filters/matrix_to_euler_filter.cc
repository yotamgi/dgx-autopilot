...
typename VecGenerator<float,3>::vector_t MatrixToEulerFilter::get_data() {
	...
	typename VecGenerator<float,3>::vector_t ans;

	const lin_algebra::mat_row plan_x = lin_algebra::mat_row(rot_mat, 0);
	const lin_algebra::mat_row plan_y = lin_algebra::mat_row(rot_mat, 1);
	const lin_algebra::mat_row plan_z = lin_algebra::mat_row(rot_mat, 2);
...
//	ans[0] = -asin(rot_mat(2,1)) * 180. / lin_algebra::PI;
//	ans[1] = -atan2(rot_mat(2,0), rot_mat(2,2)) * 180. / lin_algebra::PI;
//	ans[2] = atan2(rot_mat(0,1), rot_mat(1,1)) * 180. / lin_algebra::PI;
